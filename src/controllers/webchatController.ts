import {WebchatApi} from '../api/webchatApi';
...
export class WebchatController {
        ...
        this.webchatApi.getChats(params).then((res) => {
            handleResponse<IGetChatsRes[]>(res, (chats) => {
                ...
                Promise.all(chatsPromises).then((res) => {
                    const unreadMsgsCountArr = res.map(
                        (resp) => (JSON.parse(resp.response) as IGetUnreadMsgsRes).unread_count
                    );
    ...
    public addUserToChat({login}: IAddUserData): void {
        const selectedChat = (store.getValue('chats') as IChat[]).find((chat) => chat.isSelected);
